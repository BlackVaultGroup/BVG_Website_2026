...
import { getServiceArea, SERVICE_AREAS } from "@/content/service-areas"
...
export function ServiceAreaPage() {
  ...
  return <>
    ...
    <main className="min-h-screen bg-bv-bg-primary pt-[72px]">
      ...
      {isHub && <section className="mx-auto max-w-[1100px] px-6 py-20 md:py-28"><p className="font-body text-xs font-medium uppercase tracking-[0.16em] text-bv-accent">SERVICE AREAS</p><h2 className="mt-4 font-display text-[clamp(2rem,4vw,3rem)] font-normal text-bv-text-primary">Explore Hampton Roads locations</h2><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{SERVICE_AREAS.filter((item) => item.slug !== area.slug).map((item) => <Link key={item.slug} to={`/${item.slug}`} className="border border-border bg-bv-bg-secondary p-5 font-body text-base text-bv-text-primary no-underline transition-colors hover:border-bv-accent hover:text-bv-accent">AI consulting in {item.name} <span aria-hidden="true">→</span></Link>)}</div></section>}
    </main>
    <Footer />
  </>
}
