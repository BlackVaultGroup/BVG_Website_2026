...
export function ServiceAreaPage() {
  ...
  return <>
    ...
    <main className="min-h-screen bg-bv-bg-primary pt-[72px]">
      ...
      {isHub && <section className="mx-auto max-w-[1100px] px-6 py-20 md:py-28"><p className="font-body text-xs font-medium uppercase tracking-[0.16em] text-bv-accent">HOW THE REGION WORKS</p><h2 className="mt-4 max-w-[800px] font-display text-[clamp(2rem,4vw,3rem)] font-normal text-bv-text-primary">Different cities. Different operational pressure.</h2><p className="mt-5 max-w-[760px] font-body text-base font-light leading-[1.8] text-bv-text-secondary">The goal is not to force every business into the same AI system. It is to understand the work your team repeats and build the right level of support around it.</p><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{SERVICE_AREAS.filter((item) => item.slug !== area.slug).map((item) => <Link key={item.slug} to={`/${item.slug}`} className="border border-border bg-bv-bg-secondary p-5 no-underline transition-colors hover:border-bv-accent"><p className="font-body text-base font-medium text-bv-text-primary">{item.name}</p><p className="mt-3 font-body text-sm font-light leading-[1.7] text-bv-text-secondary">{item.hubFocus}</p><span className="mt-5 inline-block font-body text-sm text-bv-accent">Explore {item.name} <span aria-hidden="true">→</span></span></Link>)}</div></section>}
    </main>
    <Footer />
  </>
}
